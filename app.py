...
def main():
    ...
    # 사용자 입력
    term1 = st.number_input('첫 번째 항', step=1, format="%.0f")
    term2 = st.number_input('두 번째 항', step=1, format="%.0f")
    term3 = st.number_input('세 번째 항', step=1, format="%.0f")
    term4 = st.number_input('네 번째 항', step=1, format="%.0f")
    ...
    if st.button('검색'):
        try:
            ...
            if all(isinstance(term, (int, float)) and term.is_integer() for term in terms):
